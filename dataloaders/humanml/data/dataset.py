...
from dataloaders.humanml.utils.word_vectorizer import WordVectorizer
from dataloaders.humanml.utils.get_opt import get_opt
...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        Args:
            ...
            split_file (_type_): _description_
            w_vectorizer (_type_): _description_
        Returns:
            - word_embeddings: tenosr[B, L=20+2, E=300]
            - pos_one_hot: tensor[B, L=22, 15]
            - caption: tuple 32 str
            - sent_len: torch[B,]
            - motion: [B, L=196, R=263]
            - m_length: torch[B, ]
            - tokens: tuple [32] str useless
        """
        self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        ...
        self.max_kf_length = self.opt.max_kf_length
        min_motion_len = 64 if self.opt.dataset_name =='t2m' else 24
        ...
        for name in tqdm(id_list):
            try:
                motion = np.load(pjoin(opt.motion_dir, name + '.npy'))
                keyframe_ids = np.load(pjoin(opt.kf_dir, name + '.npy'),allow_pickle=True)
                if (len(motion)) < min_motion_len or (len(motion) >= 200):
                    continue
                text_data = []
                flag = False
                ...
                with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:
                    for id, line in enumerate(f.readlines()):
                        ...
                        if f_tag == 0.0 and to_tag == 0.0:
                            ...
                            try:
                                n_motion = motion[int(f_tag*20) : int(to_tag*20)]
                                keyframe = np.zeros((self.max_motion_length, motion.shape[-1]))
                                keyframe[list(range(len(keyframe_ids[id]))),:] = motion[keyframe_ids[id],:]
                                keyframe_id = keyframe_ids[id].tolist()
                                keyframe_id = keyframe_id + [-1 for _ in range(self.max_kf_length-len(keyframe_id))]
                                if (len(n_motion)) < min_motion_len or (len(n_motion) >= 200):
                                    continue
                                new_name = random.choice('ABCDEFGHIJKLMNOPQRSTUVW') + '_' + name
                                while new_name in data_dict:
                ...
                if flag:
                    keyframe = np.zeros((self.max_motion_length, motion.shape[-1]))
                    keyframe[list(range(len(keyframe_ids[flag_id[0]]))),:] = motion[keyframe_ids[flag_id[0]],:]
                    keyframe_id = keyframe_ids[flag_id[0]].tolist()
                    keyframe_id = keyframe_id + [-1 for _ in range(self.max_kf_length-len(keyframe_id))]
                    data_dict[name] = {'motion': motion,
                                       'length': len(motion),
                                       'text': text_data,
    ...
    def __getitem__(self, item):
        idx = self.pointer + item
        data = self.data_dict[self.name_list[idx]]
        motion, m_length, text_list, keyframe, keyframe_id = data['motion'], data['length'], data['text'], data['keyframe'], data['keyframe_id']
        # Randomly select a caption
        text_data = random.choice(text_list)
        caption, tokens = text_data['caption'], text_data['tokens']
        ...
        last_id, last_index = next(
            ((x, len(keyframe_id)-1-i) for i,x in enumerate(reversed(keyframe_id)) if x != -1), 
            (None, None))
        # if last_id >= len(motion):
        #     keyframe_id[last_index] = len(motion)-1
        current_in = last_index
        while current_in >=0:
            if keyframe_id[last_index] < len(motion):
                break
            else:
                keyframe_id[last_index] = -1
                last_index = last_index - 1
        
        # if max(keyframe_id) >= len(motion):
        #     print(self.name_list[idx], last_index)
        #     print(keyframe_id, motion.shape, len(motion))
        keyframe = np.zeros_like(keyframe)
        keyframe[:last_index+1] = motion[keyframe_id[:last_index+1]]
        ...
        # print(tokens)
        return word_embeddings, pos_one_hots, caption, sent_len, motion, m_length, '_'.join(tokens), keyframe, keyframe_id
...
class Text2MotionDatasetBaseline(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        self.pointer = 0
        self.max_motion_length = opt.max_motion_length
        min_motion_len = 60 if self.opt.dataset_name =='t2m' else 24
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", **kwargs):
        ...
        if mode == 'text_only':
            ...
            self.w_vectorizer = WordVectorizer(pjoin(abs_base_path, 'pretraining/glove'), 'our_vab')
    ...
    def renorm4t2m(self, data):
        if self.mode == 'eval':
            denormed_data = self.t2m_dataset.inv_transform(data)
            renormed_data = (denormed_data - self.mean_for_eval) / self.std_for_eval  
            # according to T2M norms
        return renormed_data
# A wrapper class for t2m original dataset for MDM purposes
class KIT(HumanML3D):
