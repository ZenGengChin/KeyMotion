import torch
from dataloaders.humanml.networks.modules import *
#from dataloaders.humanml.networks.trainers import CompTrainerV6
...
class CHMGGeneratedDataset(Dataset):
    """
    ...
    def __init__(self, 
        ...
        self.w_vectorizer = self.dataset.w_vectorizer
        
        self.data_list = [] # list of all dict like result
        for idx, data in enumerate(self.dataset):
            tmp = list(data)
            motion_in = data[4][None,:,:]
            motion_est = self.model(torch.Tensor(motion_in)).squeeze().cpu().numpy()
            
            if self.dataset.mode == 'eval':
                normed_motion = motion_est
                denormed_motion = self.dataset.t2m_dataset.inv_transform(normed_motion)
                renormed_motion = (denormed_motion - self.dataset.mean_for_eval) / self.dataset.std_for_eval  # according to T2M norms
                motion_est = renormed_motion
...
class IPMAEGeneratedDataset(Dataset):
    """
    The dataset for generated CHMG model.
    """
    def __init__(self, 
        ...
        for idx, data in enumerate(self.dataset):
            tmp = list(data)
            motion_in = data[4][None,:,:]
            motion_in = torch.Tensor(motion_in).to(model.device)
            lengths = torch.Tensor([data[-2]]).to(model.device)
            text = [data[2]]
            motion_est = self.model.forward(
                X = motion_in,
                text_cond = text,
                lengths=lengths,
                tween_mask=None,
            ...
            motion_est = motion_est.squeeze().cpu().detach().numpy()
            if self.dataset.mode == 'eval':
                normed_motion = motion_est
                denormed_motion = self.dataset.t2m_dataset.inv_transform(normed_motion)
...
from model.chmg.KFIP import KFIP

class KFIPGeneratedDataset(Dataset):
    """
    The dataset for generated CHMG model.
    """
    def __init__(self, 
        ...
        for idx, data in enumerate(self.dataset):
            ...
            lengths = torch.Tensor([data[-2]]).to(model.device)
            text = [data[2]]
            motion_est = self.model.forward(text=text,
                                            lengths=lengths)
...
from model.chmg.KFDM import PSKF

class PSKFGeneratedDataset(Dataset):
    """
    The dataset for generated CHMG model.
    """
    def __init__(self, 
...
from model.chmg.KFDMV2 import KeyFrameDiffusion

class KFDMGeneratedDataset(Dataset):
    """
    The dataset for generated CHMG model.
    """
    def __init__(self, 
                 model:KeyFrameDiffusion,
                 dataloader:DataLoader) -> None:
        super().__init__()
        self.model = model 
        self.dataloader = dataloader
...
class KFSDMGeneratedDataset(Dataset):
    """
    The dataset for generated CHMG model.
    """
    def __init__(self, 
        ...
        for idx, data in enumerate(self.dataset):
            ...
            lengths = torch.Tensor([data[-2]]).to(model.device)
            text = [data[2]]
            # motion_in will not be seen in diffusion part. 
            motion_est = self.model(
                motion_in,
                text,
                lengths)
