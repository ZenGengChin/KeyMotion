...
class AdjustableMeshViewer(object):

    def __init__(self, width=1200, height=800, yfov=np.pi / 3.0,use_offscreen=True):
        #super().__init__()
    ...
    def set_cam_trans(self, trans= [0, 0, 3.0], angle=[0,0,0]):
        import torch 
        from torch import nn, Tensor
        from pytorch3d.transforms import axis_angle_to_matrix
        
        if isinstance(trans, list): trans = np.array(trans)
        if isinstance(angle, list): angle = torch.Tensor(np.array(angle))
        matrix = axis_angle_to_matrix(angle).cpu().detach().numpy()
        
        camera_pose = np.eye(4)
        
        camera_pose[0:3,0:3] = matrix
